...
$content .= '<ul class="moduletable' . $moduleclass_sfx . '">';
foreach ($rows as $row) {
	if (strlen($row->title) > $wllengthoftitle) {
		$row->title = substr($row->title, 0, $wldotaddlenght);
		$row->title .= '…';
	}
	$rdate = $row->cdate;
...
if ($wlreadmore) {
	...
	$content .= '<a href="index.php?option=com_weblinks' . $catid . '&amp;Itemid=1" class="readon">' . _MORE . '</a>';
}
?>