...
// strip text if needed
		if ($params->get('ml_imaged') == 1) {
			...
			$mitem->name = '<img src="'.$mosConfig_live_site.'/templates/'.$cur_template.'/i/m/mljoostinamenu/zaglushka.gif" alt="'.$ml_alt.'" />';
		} elseif (($params->get('ml_imaged') == 2) && ($params->get('ml_aligner') == 'left')) {
			$ml_alt = $mitem->name;
			$ml_img_title = ' title="'.$pg_title.'"';
			$mitem->name = '<img src="'.$mosConfig_live_site.'/templates/'.$cur_template.'/i/m/mljoostinamenu/zaglushka.gif" alt="'.$ml_alt.'" /><em>'.$mitem->name.'</em>';
		} elseif (($params->get('ml_imaged') == 2) && ($params->get('ml_aligner') == 'right')) {
			$ml_alt = $mitem->name;
			$ml_img_title = ' title="'.$pg_title.'"';
			$mitem->name = '<em>'.$mitem->name.'</em><img src="'.$mosConfig_live_site.'/templates/'.$cur_template.'/i/m/mljoostinamenu/zaglushka.gif" alt="'.$ml_alt.'" />';
		} else {
...
// None
			...
			default:
// Template
				$imgpath = $mosConfig_live_site.'/templates/'.$cur_template.'/i/m/mljoostinamenu';
				for ($i = 1; $i < 7; ++$i) {
					$img[$i] = '<img src="'.$imgpath.'/indent'.$i.'.png" alt="" />';
				}
				break;
