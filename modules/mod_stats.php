...
global $mosConfig_offset, $mosConfig_caching, $mosConfig_gzip, $mosConfig_enable_stats, $database;
...
echo '<div class="stats">';
	echo '<ul>';
		...
		if ($serverinfo) {
		// Информация о OS
		echo '<li><span class="stats-type">'. _STAT_OS.'</span> <span class="stats-item">'.mb_substr(php_uname(), 0, 10).'</span></li>';
		// Информация о PHP
	...
	$c = $mosConfig_caching ? ''._STAT_CACHE_ENABLE.'' : ''._STAT_CACHE_DISABLE.'';
		...
		// Информация о gzip
	$z = $mosConfig_gzip ? ''._STAT_GZIP_ENABLE.'' : ''._STAT_GZIP_DISABLE.'';
