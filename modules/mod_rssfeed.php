...
// проверка на отключенность/включенность выделеных типов фидов
...
// Yandex
	if ($yandex) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=Yandex&amp;no_html=1');
		output_rssfeed($link, 'yandex_rss.png', $yandex_image, 'Yandex RSS', 'Yandex');
	}
// rss091
	if ($rss091) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=RSS0.91&amp;no_html=1');
		output_rssfeed($link, 'rss091.gif', $rss091_image, 'RSS 0.91', 'RSS091');
	}
// rss10
	if ($rss10) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=RSS1.0&amp;no_html=1');
		output_rssfeed($link, 'rss10.gif', $rss10_image, 'RSS 1.0', 'RSS10');
	}
// rss20
	if ($rss20) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=RSS2.0&amp;no_html=1');
		output_rssfeed($link, 'rss20.gif', $rss20_image, 'RSS 2.0', 'RSS20');
	}
// atom
	if ($atom03) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=ATOM0.3&amp;no_html=1');
		output_rssfeed($link, 'atom03.gif', $atom_image, 'ATOM 0.3', 'ATOM03');
	}
// opml
	if ($opml) {
		$link = sefRelToAbs('index.php?option=com_rss&amp;feed=OPML&amp;no_html=1');
