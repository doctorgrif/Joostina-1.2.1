 ...
 */
require_once($mosConfig_absolute_path.'/includes/openid/Auth/OpenID/Extension.php' );
...
/**
 * Given a list of authentication policy URIs that a provider supports, this method returns the subsequence of those types
 * that are preferred by the relying party.
 * @param supported_types: A sequence of authentication policy type URIs that are supported by a provider
 * @return array The sub-sequence of the supported types that are preferred by the relying party. This list will be ordered in
 * the order that the types appear in the supported_types sequence, and may be empty if the provider does not prefer any
 ...
 * This method is intended to be used by the provider to add a policy that the provider conformed to when authenticating the
 * user.
 * @param policy_uri: The identifier for the preferred type of authentication.
 */
function addPolicyURI($policy_uri)
{
 ...
 * @param success_response $success_response A SuccessResponse from Auth_OpenID_Consumer::complete()
 * @returns: A provider authentication policy response from the data that was supplied with the id_res response.
 */
 ...
 * @param args: unqualified provider authentication policy arguments
 * @param strict: Whether to return false when bad data is encountered
