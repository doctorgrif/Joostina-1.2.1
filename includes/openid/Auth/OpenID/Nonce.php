...
/**
 * Keep nonces for five hours (allow five hours for the combination of request time and clock skew). This is probably way more
 * than is necessary, but there is not much overhead in storing nonces.
*/
...
// the stamp is not too far in the future and is not too far in the past
...
if ($when === null) {
// It's safe to call time() with no arguments; it returns a GMT unix timestamp on PHP 4 and PHP 5.gmmktime() with no
// args returns a local unix timestamp on PHP 4, so don't use that.
$when = time();
}
