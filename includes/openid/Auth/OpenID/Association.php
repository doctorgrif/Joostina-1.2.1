...
*/
/** @access private */
require_once($mosConfig_absolute_path.'/includes/openid/Auth/OpenID/CryptUtil.php' );
/** @access private */
require_once($mosConfig_absolute_path.'/includes/openid/Auth/OpenID/KVForm.php' );
/** @access private */
require_once($mosConfig_absolute_path.'/includes/openid/Auth/OpenID/HMAC.php' );
/**
* This class represents an association between a server and a consumer.  In general, users of this library will never see
* instances of this object.  The only exception is if you implement a custom {@link Auth_OpenID_OpenIDStore}.
* If you do implement such a store, it will need to store the values of the handle, secret, issued, lifetime, and assoc_type instance
* variables.
* @package OpenID
*/
...
/**
 * This is an alternate constructor (factory method) used by the OpenID consumer library to create associations.  OpenID store
 * implementations shouldn't use this constructor.
 * @access private
 * @param integer $expires_in This is the amount of time this association is good for, measured in seconds since the
 * association was issued.
 * @param string $handle This is the handle the server gave this association.
 * @param string secret This is the shared secret the server generated for this association.
 * @param assoc_type This is the type of association this instance represents.  The only valid values of this field at
 * this time is 'HMAC-SHA1' and 'HMAC-SHA256', but new types may be defined in the future.
 ...
 * @param string $handle This is the handle the server gave this association.
 * @param string $secret This is the shared secret the server generated for this association.
 * @param integer $issued This is the time this association was issued, in seconds since 00:00 GMT, January 1, 1970.  (ie, a
 * unix timestamp)
 * @param integer $lifetime This is the amount of time this association is good for, measured in seconds since the
 * association was issued.
 * @param string $assoc_type This is the type of association this instance represents.  The only valid values of this field at
 * this time is 'HMAC-SHA1' and 'HMAC-SHA256', but new types may be defined in the future.
 */
 ...
 * @return bool $result true if the two instances represent the same association, false otherwise.
 */
 ...
 * @access private
 * @param array $pairs The pairs to sign, in order.  This is an array of two-tuples.
 ...
 * @access private
 * @param array $fields The fields to sign, in order; this is an array of strings.
 * @param array $data Dictionary of values to sign (an array of string => string pairs).
...
/**
 * Given a {@link Auth_OpenID_Message}, return the key/value pairs to be signed according to the signed list in the message.  If
...
/**
 * A session negotiator controls the allowed and preferred association types and association session types. Both the {@link
 * Auth_OpenID_Consumer} and {@link Auth_OpenID_Server} use negotiators when creating associations.
 ...
 * The server gets a request for a particular association/session type and calls {@link isAllowed} to determine if it should create an
 * association. If it is supported, negotiation is complete. If it is not, the server calls {@link getAllowedType} to get an allowed
 * association type to return to the consumer.
 * If the consumer gets an error response indicating that the requested association/session type is not supported by the server
...
/**
 * Add an association type and session type to the allowed types list. The assocation/session pairs are tried in the order that
 * they are added.
 * @access private
 */
