...
*/
/**
 * This module implements a VERY limited parser that finds <link> tags in the head of HTML or XHTML documents and parses out their
 * attributes according to the OpenID spec. It is a liberal parser, but it requires these things from the data in order to work:
 * - There must be an open <html> tag
 * - There must be an open <head> tag inside of the <html> tag
 * - Only <link>s that are found inside of the <head> tag are parsed(this is by design)
 * - The parser follows the OpenID specification in resolving the attributes of the link tags. This means that the attributes DO
 * NOT get resolved as they would by an XML or HTML parser. In particular, only certain entities get replaced, and href
 ...
 * - The <head> tag is accepted even when it is not a direct child of the <html> tag, but a <html> tag must be an ancestor of the
 * <head> tag
 * - <link> tags are accepted even when they are not direct children of the <head> tag, but a <head> tag must be an ancestor of the
 * <link> tag
 * - If there is no closing tag for an open <html> or <head> tag, the remainder of the document is viewed as being inside of the
 * tag. If there is no closing tag for a <link> tag, the link tag is treated as a short tag. Exceptions to this rule are that <html>
 ...
 * - Any text that does not parse as an attribute within a link tag will be ignored. (e.g. <link pumpkin rel='openid.server' /> will
 * ignore pumpkin)
