...
*/
require_once($mosConfig_absolute_path.'/includes/openid/Services/Yadis/Misc.php' );
require_once($mosConfig_absolute_path.'/includes/openid/Services/Yadis/Yadis.php' );
...
{
// Append some arguments to an HTTP query.Yes, this is just like OpenID's appendArgs, but with special seasoning for XRI queries.
if (count($args) == 0) {
return $url;
}
...
// According to XRI Resolution section "QXRI query parameters": "If the original QXRI had a null query component (only a leading question mark), or a query component consisting of only question marks, one additional leading question mark MUST be added when adding any XRI resolution parameters."
...
if ($authority[0] == '(') {
// Cross-reference.
// XXX: This is incorrect if someone nests cross-references so there is another close-paren in there.Hopefully nobody does that before we have a real xriparse function.
