...
require_once($mosConfig_absolute_path.'/includes/openid/Services/Yadis/ParseHTML.php' );
...
// The URI from which the response text was returned (set to None if there was no XRDS document found)
var $xrds_uri = null;
var $xrds = null;
...
/**
 * Returns the list of service objects as described by the XRDS document, if this yadis object represents a successful Yadis
 * discovery.
 ...
 * @return: The normalized identity URL and an iterable of endpoint objects generated by the filter function.
 * xrds_parse_func: a callback which will take (uri, xrds_text) and return an array of service endpoint objects or null.Usually
...
/**
 * This is the core of the PHP Yadis library.This is the only class a user needs to use to perform Yadis discovery. This class
 ...
 * If the discovery succeeds, $yadis_object will be an instance of {@link Auth_Yadis_Yadis}. If not, it will be null.The XRDS
 * document found during discovery should have service descriptions, which can be accessed by calling
 * <pre>$service_list = $yadis_object->services();</pre>
 * which returns an array of objects which describe each service.
 * These objects are instances of Auth_Yadis_Service. Each object describes exactly one whole Service element, complete with all of
 * its Types and URIs (no expansion is performed). The common use case for using the service objects returned by services() is to
 ...
 * <pre>function myFilter(&$service) {Query $service object here.Return true if the service matches your query; false if not.}</pre>
 * This is an example of a filter which uses a regular expression to match the content of URI tags (note that the Auth_Yadis_Service
 ...
 *return false;
 *}</pre>
 * The filter functions you pass will be called for each service object to determine which ones match the criteria your filters
 * specify.The default behavior is that if a given service object matches ANY of the filters specified in the services() call, it
 * will be returned.You can specify that a given service object will be returned ONLY if it matches ALL specified filters by changing
 * the match mode of services():
 * <pre>$yadis_object->services(array("filter1", "filter2"), SERVICES_YADIS_MATCH_ALL);</pre>
 * See {@link SERVICES_YADIS_MATCH_ALL} and {@link SERVICES_YADIS_MATCH_ANY}.
 * Services described in an XRDS should have a library which you'll probably be using.Those libraries are responsible for defining
 * filters that can be used with the "services()" call.If you need to write your own filter, see the documentation for {@link
 * Auth_Yadis_Service}.
 * @package OpenID
 */
class Auth_Yadis_Yadis {
/**
 * Returns an HTTP fetcher object.If the CURL extension is present, an instance of {@link Auth_Yadis_ParanoidHTTPFetcher}
 * is returned.If not, an instance of {@link Auth_Yadis_PlainHTTPFetcher} is returned.
...
/**
 * This should be called statically and will build a Yadis instance if the discovery process succeeds.This implements
 * Yadis discovery as specified in the Yadis specification.
 * @param string $uri The URI on which to perform Yadis discovery.
 * @param array $http_response An array reference where the HTTP response object will be stored (see {@link Auth_Yadis_HTTPResponse}.
 * @param Auth_Yadis_HTTPFetcher $fetcher An instance of a Auth_Yadis_HTTPFetcher subclass.
 * @param array $extra_ns_map An array which maps namespace names to namespace URIs to be used when parsing the Yadis XRDS
 * document.
