...
*/
/**
 * The base session class used by the Auth_Yadis_Manager.This class wraps the default PHP session machinery and should be
...
/**
 * A session helper class designed to translate between arrays and objects.Note that the class used must have a constructor that
 * takes no parameters.This is not a general solution, but it works for dumb objects that just need to have attributes set.The idea
 * is that you'll subclass this and override $this->check($data) -> bool to implement your own session data validation.
 * @package OpenID
 */
...
/**
 * Given a session data value (an array), this creates an object (returned by $this->newObject()) whose attributes and values
 * are those in $data.Returns null if $data lacks keys found in $this->requiredKeys().Returns null if $this->check($data)
 * evaluates to false.Returns null if $this->newObject() evaluates to false.
 * @access private
 */
...
/**
 * Returns a new instance of this loader's class, using the session data to construct it if necessary.The object need
 * only be created; $this->fromSession() will take care of setting the object's attributes.
 * @access private
 */
...
/**
 * Returns an array of keys and values built from the attributes of $obj.If $this->prepareForSave($obj) returns an array, its keys
 * and values are used to update the $data array of attributes from $obj.
 * @access private
 */
...
/**
 * The Yadis service manager which stores state in a session and iterates over <Service> elements in a Yadis XRDS document and lets
 * a caller attempt to use each one.This is used by the Yadis library internally.
 * @package OpenID
 */
 ...
 * High-level usage pattern is to call .getNextService(discover) in order to find the next available service for this user for this
 * session. Once a request completes, call .cleanup() to clean up the session state.
 * @package OpenID
 */
 ...
 * @param Auth_Yadis_PHPSession $session An object which implements the Auth_Yadis_PHPSession API.
 * @param string $url The URL on which to attempt discovery.
 * @param string $session_key_suffix The optional session key suffix override.
 */
 ...
 * @param $force True if the manager should be deleted regardless of whether it's a manager for $this->url.
 */
...
/**
 * @access private
 * @param $force True if the manager should be returned regardless of whether it's a manager for $this->url.
 */
