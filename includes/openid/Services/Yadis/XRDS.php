...
require_once($mosConfig_absolute_path.'/includes/openid/Services/Yadis/XML.php' );
 ...
 * Objects of this type are returned by Auth_Yadis_XRDS::services() and
 * Auth_Yadis_Yadis::services().Each object corresponds directly to a <Service> element in the XRDS and supplies a
 * getElements($name) method which you should use to inspect the element's contents.See {@link Auth_Yadis_Yadis} for more
 ...
 * This is what you should use to get all custom information out of this element. This is used by service filter functions to
 * determine whether a service element contains specific tags, etc.NOTE: this only considers elements which are direct
 * children of the <Service> element for this object.
 * @param string $name The name of the element to look for
 * @return array $list An array of elements with the specified name which are direct children of the <Service> element.The
 ...
 * @param $default The value to use as the expiration if no expiration was specified in the XRD.
 */
 ...
 * If the XRDS can be parsed and is valid, an instance of Auth_Yadis_XRDS will be returned.Otherwise, null will be
 * returned.This class is used by the Auth_Yadis_Yadis::discover method.
 * @package OpenID
 */
...
/**
 * Parse an XML string (XRDS document) and return either a Auth_Yadis_XRDS object or null, depending on whether the
 * XRDS XML is valid.
 ...
 * Optionally, an array of filter callbacks may be given to limit the list of returned service objects.Furthermore, the default
 * mode is to return all service objects which match ANY of the specified filters, but $filter_mode may be
 * SERVICES_YADIS_MATCH_ALL if you want to be sure that the returned services match all the given filters.See {@link
 * Auth_Yadis_Yadis} for detailed usage information on filter functions.
 * @param mixed $filters An array of callbacks to filter the returned services, or null if all services are to be returned.
 * @param integer $filter_mode SERVICES_YADIS_MATCH_ALL or SERVICES_YADIS_MATCH_ANY, depending on whether the returned
 * services should match ALL or ANY of the specified filters, respectively.
 * @return mixed $services An array of {@link Auth_Yadis_Service} objects if $filter_mode is a valid
 * mode; null if $filter_mode is an invalid mode (i.e., not SERVICES_YADIS_MATCH_ANY or SERVICES_YADIS_MATCH_ALL).
 */
...
// If no filters are specified, return the entire service list, ordered by priority.
if (!$filters ||
(!is_array($filters))) {
$result = array();
...
// Otherwise, use the callbacks in the filter list to determine which services are returned.
$filtered = array();
