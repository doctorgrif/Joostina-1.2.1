...
$playbutton.attr({title:'Запустить',src:simpleGallery_navpanel.images[1]})}
...
setting.gallerylayers[setting.fglayer].style.zIndex=99
...
layerHTML+='<img src="'+imgelement[0]+'" alt="slideshow_img" />'
layerHTML+=(imgelement[1])?'</a>':''
return layerHTML},addnavpanel:function(setting){var interfaceHTML=''
for(var i=0;i<3;i++){var imgstyle='position:relative;cursor:hand;cursor:pointer;top:'+simpleGallery_navpanel.imageSpacing.offsetTop[i]+'px;margin-right:'+(i!=2?simpleGallery_navpanel.imageSpacing.spacing+'px':0)
var title=(i==0?'Предыдущая':(i==1)?(setting.ispaused?'Запустить':'Пауза'):'Следующая')
