...
class FeedCreator extends HtmlDescribable {
	...
	function iTrunc($string, $length) {
		...
		if ($pos >= $length * 0.4) {
			return substr($string, 0, $pos + 1) . "Е";
		}
		$pos = strrpos($string, " ");
		if ($pos >= $length - 4) {
			$string = substr($string, 0, $length - 4);
		...
		if ($pos >= $length * 0.4) {
			return substr($string, 0, $pos) . " Е";
		}
		return substr($string, 0, $length - 4) . " Е";
	}
...
//$now = new FeedDate();
		...
		for ($i = 0; $i < count($this->items); ++$i) {
			$feed .= "<item>\n";
			$feed .= "<title>" . FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)), 100) . "</title>\n";
// при отключенном SEF допишем к адресу ленты полный путь от корн€
