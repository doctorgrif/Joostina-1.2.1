	...
	function autoCompressJavascript($sJsFullFilename = null) {
		...
		if(!file_exists($realJsFile)) {
			...
			if(!file_exists($srcFile)) {
				trigger_error("The xajax uncompressed Javascript file could not be found in the <strong>".
					dirname($realJsFile)."</strong> folder. Error ",E_USER_ERROR);
			}
			...
			if(!$fH) {
				trigger_error("The xajax compressed javascript file could not be written in the <strong>".
					dirname($realJsFile)."</strong> folder. Error ",E_USER_ERROR);
			} else {
