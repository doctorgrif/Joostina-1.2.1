...
if (defined('_VALID_MOS')) {//если запуск в среде Joo..., дозагружаем параметры мамбота
	...
	if (isset($j_spaw_config['user_dir'])) 
    if ($j_spaw_config['user_dir'] == 1 && 'Manager' != $my->usertype && 'Administrator' != $my->usertype && 'Super Administrator' != $my->usertype) {
    ...
    $tempcss = '';
	if (isset($j_spaw_config['template'])) 
    if ($j_spaw_config['template'] == 1) { //если используем css шаблона
        $tempcss = $mosConfig_live_site . '/templates/' . $mainframe->getTemplate() . '/css/style.css';
        if (!file_exists($tempcss)) {//для FrosTPK : )
            $database->setQuery("SELECT template FROM #__templates_menu WHERE client_id='0' AND menuid='0'");
            $tempcss = $mosConfig_live_site . '/templates/' . $database->loadResult() . '/css/style.css';
        }
    }
    if ($tempcss)
