...
function js_menu_cache($data, $usertype, $state = 0) {
	...
	if (!file_exists($file)) { // файла нету
		if ($state == 1)
			return false; // файла у нас не было и получен сигнал 0 - продолжаем вызывающую функцию, а отсюда выходим
		touch($file);
		$handle = fopen($file, 'w');
		fwrite($handle, $data);
		fclose($handle);
...
function josSecurityCheck($width = '95%') {
	...
	if (!is_writeable($mosConfig_cachepath) && $mosConfig_caching)
		$wrongSettingsTexts[] = _CACHE_DIR_IS_NOT_WRITEABLE2;
	// проверка magic_quotes_gpc
	if (ini_get('magic_quotes_gpc') != '1')
		$wrongSettingsTexts[] = _PHP_MAGIC_QUOTES_ON_OFF;
	// проверка регистрации глобальных переменных
	if (ini_get('register_globals') == '1')
		$wrongSettingsTexts[] = _PHP_REGISTER_GLOBALS_ON_OFF;
	// проверка активированности внутренней системы защиты
	if (RG_EMULATION != 0)
		$wrongSettingsTexts[] = _RG_EMULATION_ON_OFF;
	...
	if (count($wrongSettingsTexts)) {
		...
		<?php echo _PHP_SETTINGS_WARNING ?>:
			...
			<ul style="margin:0;padding:0;padding-left:15px;list-style:none;" >
		<?php
