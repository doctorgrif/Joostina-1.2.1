...
if (!class_exists('FPDF_JOOPDF')) {
	define('FPDF_JOOPDF_VERSION', '1.6');
	...
	class FPDF_JOOPDF {
		...
		var $imageScale; // Standard scaling of images
		var $font_path; // The font path to use

		var $traceBuffer = '';
		var $enableTrace = false;
		/* Public methods */
		function FPDF_JOOPDF($orientation = 'P', $unit = 'mm', $format = 'A4') {
			//Some checks
			$this->_dochecks();
			...
			if (is_string($format)) {
				...
				$this->Error('Unknown page format: ' .
				$format);
				$this->fwPt = $format[0];
				$this->fhPt = $format[1];
			} else {
			...
			$this->PDFVersion = '1.3';
			$this->imageScale = 1.0; // full size images
		...
		function AddFont($family, $style = '', $file = '') {
			...
			if ($file == '')
			$file = str_replace(' ', '', $family) .
			strtolower($style) . '.php';
			if ($family == 'arial')
			$family = 'helvetica';
		...
		function SetFontx($family, $style = '', $size = 0) {
		...
		function Write($h, $txt, $link = '', $align = '') {
			...
			if ($i != $j) {
				...
				//		$this->Cell(0,$h,substr($s,$j),0,0,$align,0,$link);
			}
		}
		...
		function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '', $isMask = false, $maskImg = 0) {
			...
			if (!isset ($this->images[$file])) {
				...
				if ($type == 'jpg' || $type == 'jpeg') {
					...
					if ($info == 'alpha')
					$info['data'] = $this->ImagePngWithAlpha($file, $x, $y, $w, $h, $link);
				} elseif ($type == 'gif') {
			...
			if ($w == 0 && $h == 0) {
				//Put image at 72 dpi
				$w = ($info['w'] / $this->k) * $this->imageScale;
				$h = ($info['h'] / $this->k) * $this->imageScale;
			}

			if ($w == 0)
			$w = ($h * $info['w'] / $info['h']) * $this->imageScale;
			if ($h == 0)
			$h = ($w * $info['h'] / $info['w']) * $this->imageScale;

			if ($isMask)
			$x = $this->fwPt + 10; // embed hidden, outside the canvas
			if ($x == -1) {
				// Center image on page
				$x = (($this->width - $w) / 2) + $this->left;
			}
			elseif ($x == -2) {
				// Align image Left
				$x = $this->left;
				...
				case 'I' :
					...
					if (php_sapi_name() != 'cli') {
						if (!headers_sent()) {
							...
							// $this->Error('Some header data has already been output to browser, can\'t send PDF file');
		...
		function _getfontpath() {
			...
			if ($this->font_path == '' && is_dir(dirname(__FILE__) . '/font'))
			$this->setFontpath(dirname(__FILE__) . '/font/');
			return $this->font_path;
		}

		function setFontpath($path = '') {
			if ($path == '' && is_dir(dirname(__FILE__) . '/font')) {
				$this->font_path = dirname(__FILE__) . '/font/';
			}
			elseif (is_dir($path)) {
				$this->font_path = $path;
			}
		...
		function _putfonts() {
			...
			foreach ($this->fonts as $k => $font) {
				...
				elseif ($type == 'Type1' || $type == 'TrueType') {
					...
					if ($file)
					$s .= ' /FontFile' .
					($type == 'Type1' ? '' : '2') . ' ' . $this->FontFiles[$file]['n'] . ' 0 R';
					$this->_out($s . '>>');
					$this->_out('endobj');
				} else {
		...
		function _putinfo() {
			$this->_out('/Producer ' . $this->_textstring('FPDF ' . FPDF_JOOPDF_VERSION));
			if (!empty ($this->title))
			$this->_out('/Title ' .
		...
		function _parsegif($file) {
			$this->trace('_parsegif() gif-file: ' . $file);
			//Extract info from a JPEG file
			$a = @ GetImageSize($file);
			if (!$a) {
				$this->trace('GetImageSize failed on: ' . $file);
				return null;
				...
				$f = fopen($file, 'rb');
				if (!$f) {
					$this->trace('!!! Failed to open gif-file: ' . $file);
				}
				$data = '';
				while (!feof($f))
				$data .= fread($f, 4096);
				...
				require_once(dirname(__FILE__) . '/gif.php');
				...
				$res = $gif->loadData($data, 0);
				if (!$res) {
					$this->trace('_parsegif(): unable to parse file ' . $file);
					return array (
					'w' => 0,
					'h' => 0,
					'cs' => 0,
				...
				$this->trace('_parsegif() GIF loaded');
		...
		function trace($info, $struct = null) {

			if ($this->enableTrace) {
				$this->traceBuffer .= '<br />' . $info;
				if (isset ($struct)) {
					ob_start();
					print_r($struct);
					$out = ob_get_contents();
					ob_end_clean();
					$this->traceBuffer .= 'struct=' . $out;
				}
			}
		}
		...
		function writeTrace() {
			if ($this->enableTrace) {
				$this->AddPage();
				$this->WriteHtml($this->traceBuffer);
			}
		}
