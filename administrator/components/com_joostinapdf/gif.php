	...
	function load($data, & $datLen) {
		...
		while (true) {
			...
			switch ($b) {
				...
				case 0x21 : // Extension
					$len = $this->skipExt($data);
					$datLen += $len;
					break;

				case 0x2C : // Image
					// LOAD HEADER & COLOR TABLE
					$len = $this->m_gih->load($data);
					if ($len < 0) {
						return -1;
					}
...
class CGIF {
	...
	function loadData($GIFData, $iIndex) {
		if ($iIndex < 0) {
			return false;
		}
		$this->m_lpData = $GIFData;

		// GET FILE HEADER
		$len = $this->m_gfh->load($this->m_lpData, $len = 0);
		if ($len < 0) {
			return false;
		}
		...
		do {
			$len = $this->m_img->load($this->m_lpData, $imgLen = 0);
			if ($len < 0) {
				return false;
			}
			$this->m_lpData = substr($this->m_lpData, $len);
