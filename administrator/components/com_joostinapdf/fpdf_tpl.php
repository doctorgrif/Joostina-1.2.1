...
class fpdf_tpl extends FPDF_JOOPDF {
	/**
	 * Array of Tpl-Data
	 * @var array
	 */
	...
	function fpdf_tpl($orientation = 'P', $unit = 'mm', $format = 'A4') {
		parent :: fpdf_joopdf($orientation, $unit, $format);
	}
	...
	function SetFont($family, $style = '', $size = 0) {
		...
		if (!isset ($this->fonts[$fontkey])) {
			...
			if (isset ($this->CoreFonts[$fontkey])) {
				if (!isset ($fpdf_charwidths[$fontkey])) {
					...
					$fontfile .= '.php';
					$fontfile = $this->_getFontpath() . $fontfile;
					include ($fontfile);
					if (!isset ($fpdf_charwidths[$fontkey])) {
						echo 'tried:' . $fontkey;
						print_r($this->fonts);
						$this->Error('Could not include font metric file ' . $fontfile);
					}
				}
		...
		if ($this->page > 0) {
			$this->_out(sprintf('BT ' .
			$this->fontprefix . '%d %.2f Tf ET', $this->CurrentFont['i'], $this->FontSizePt));
		}
		if ($this->intpl) {
	...
	function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '') {
		if (parent :: Image($file, $x, $y, $w, $h, $type, $link) != null) {
			if ($this->intpl) {
