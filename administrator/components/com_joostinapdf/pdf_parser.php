	  	...
	  	switch ($token) {
			case '<':
				...
				while(1) {
					...
					// If you can't find it, try reading more data from the stream
			...
			case	'('		:
				...
				while(1) {
					...
					// Start by finding the next closed parenthesis
					...
					// Make sure that there is no backslash before the parenthesis. If there is, move on.
				...
				if (is_numeric ($token)) {
					...
					if (($tok2 = $this->pdf_read_token ($c)) !== false) {
						if (is_numeric ($tok2)) {
							...
							// In this case, we're probably in front of either an object reference or an object specification.
							...
							if (($tok3 = $this->pdf_read_token ($c)) !== false) {
								...
								// If we get to this point, that numeric value up there was just a numeric value. Push the extra
	...
	function pdf_resolve_object(&$c, $obj_spec, $encapsulate = true) {
		...
		if ($obj_spec[0] == PDF_TYPE_OBJREF) {
			...
			if (isset($this->xref['xref'][$obj_spec[1]][$obj_spec[2]])) {
				...
				// This is needed if you want to resolve references while you're reading another object
				...
				// If we're being asked to store all the information about the object, we add the object ID and generation
				...
				// Now simply read the object data until we encounter an end-of-object marker
				while(1) {
	...
	*/
	function pdf_read_token(&$c)
	{
		...
		switch ($char) {
			...
			case '<':
			case '>':
				// This could either be a hex string or dictionary delimiter.
			...
			default		:
				// This is "another" type of token (probably a dictionary entry or a numeric value)
				...
				while(1) {
					...
					if ($c->offset + $pos <= $c->length - 1) {
						...
						// If the script reaches this point, the token may span beyond the end of the current buffer.
						// Therefore, we increase the size of the buffer and try again--just to be safe.
						$c->increase_length();
					}
				}
