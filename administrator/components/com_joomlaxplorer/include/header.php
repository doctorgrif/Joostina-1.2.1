	...
	if($allow && @$GLOBALS['jx_File']->is_writable( get_abs_dir( $dir)) ) {
		echo "<td align=\"right\">
				<form action=\"".make_link("mkitem",$dir,NULL)."\" method=\"post\" name=\"mkitemform\">\n
					<select class=\"textarea\" name=\"mktype\" onchange=\"checkMkitemForm(this.options[this.selectedIndex])\">
		...
		echo "</select>
					...
					<input class=\"textarea\" name=\"mkname\" type=\"text\" size=\"15\" title=\"{$GLOBALS['messages']['nameheader']}\" />
					<input class=\"textarea\" type=\"submit\" value=\"".$GLOBALS["messages"]["btncreate"]."\" />
				</form>
			</td>\n</tr>";
	}
	echo '</tbody></table>';
